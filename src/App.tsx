...
import { DOMAIN_IDEAS, DEFAULT_AFFILIATE_URL, DEFAULT_TRANSUNION_URL, DEFAULT_RELIABLE_CREDIT_URL } from './data';
...
export default function App() {
  ...
  // Navigation
  const [activePage, setActivePage] = useState<'background-check' | 'transunion-credit-check' | 'reliable-credit-score'>('background-check');
  ...
  useEffect(() => {
    const handleHashChange = () => {
      ...
      if (hash === '#credit-check') {
        ...
        document.title = `${currentDomain} | Reliable Credit Score $1 Trial`;
      } else {
  ...
  const handleDomainChange = (domain: string) => {
    ...
    if (activePage === 'transunion-credit-check') {
      ...
      document.title = `${domain} | Reliable Credit Score $1 Trial`;
    } else {
      document.title = `${domain} | Rental Online Background checks`;
    }
  };

  const handleChangePage = (page: 'background-check' | 'transunion-credit-check' | 'reliable-credit-score') => {
  ...
  const handleSaveUrls = (bgUrl: string, tuUrl: string, relUrl: string) => {
